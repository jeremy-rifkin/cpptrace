if (-not (Test-Path "zstd/build/cmake/build/lib/libzstd.a")) {
    mkdir zstd
    cd zstd
    git init
...
cd zstd/build/cmake/build
make install
cd ../../../..

if (-not (Test-Path "libdwarf/build/src/lib/libdwarf/libdwarf.a")) {
    mkdir libdwarf
    cd libdwarf
    git init
    git remote add origin https://github.com/jeremy-rifkin/libdwarf-lite.git
    git fetch --depth 1 origin 4f89892a51054aa38697a94cebfdab1855acc4d9 # 2.3.0
    git checkout FETCH_HEAD
    mkdir build
    cd build
